...
#        },
    ...
    # Prepare weight/exponent info for the report
    if SELF_CALIBRATING_LORE and lore_selfcal_details:
        weights_info = f"Self-Calibrating Exponents: {lore_selfcal_details.get('exponents', 'N/A')}"
        entropy_window_info = lore_selfcal_details.get('entropy_window', config['entropy_window'])
    else:
        weights_info = f"Manual Weights: {config.get('weights', 'Not Specified')}"
        entropy_window_info = config['entropy_window']
...
Entropy Window: {entropy_window_info} points
...
"""
    ...
    try:
        with open(report_path, 'w') as f:
            ...
            if SELF_CALIBRATING_LORE and lore_selfcal_details:
                params_to_save['entropy_window'] = lore_selfcal_details.get('entropy_window')
                params_to_save['exponents'] = lore_selfcal_details.get('exponents')
                params_to_save['weights_mode'] = 'Self-Calibrating'
                if 'weights' in params_to_save: del params_to_save['weights'] # Remove potentially commented-out manual weights
            else:
                params_to_save['weights_mode'] = 'Manual'
                params_to_save['entropy_window'] = config['entropy_window'] # Keep original config value
                # 'weights' key should already be in params_to_save if mode is Manual
